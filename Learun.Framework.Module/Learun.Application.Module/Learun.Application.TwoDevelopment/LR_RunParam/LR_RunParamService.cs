...
namespace Learun.Application.TwoDevelopment.LR_RunParam
{
    ...
    /// 描 述：设备运行参数
    /// </summary>
    public class LR_RunParamService : RepositoryFactory
    {
        #region  获取数据
        ...
        public IEnumerable<tb_run_infoEntity> GetPageList(Pagination pagination, string queryJson)
        {
            try
            {
                ...
                t.id,
                t1.machine_name,
                t.act_spindle_speed_0
                 t.act_spindle_override_0,
                 t.act_feed_speed_0,
                 t.act_feed_override_0,
                 t.main_prog_num,
                     t.read_time
                ");
                strSql.Append("  FROM tb_run_info t ");
        ...
        public tb_machine_infoEntity Gettb_machine_infoEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository("BaseDb1").FindEntity<tb_machine_infoEntity>(t=>t.id == keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public tb_run_infoEntity Gettb_run_infoEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository("BaseDb1").FindEntity<tb_run_infoEntity>(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
