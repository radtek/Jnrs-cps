    ...
    public class Plan_DayService : RepositoryFactory
    {
        ...
        public IEnumerable<tb_plan_day_dataEntity> GetPageList(Pagination pagination, string queryJson)
        {
            try
            {
                ...
                return this.BaseRepository("BaseDb1").FindList<tb_plan_day_dataEntity>(strSql.ToString(),dp, pagination);
            }
            catch (Exception ex)
            {
        ...
        public tb_plan_day_dataEntity Gettb_plan_day_dataEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository("BaseDb1").FindEntity<tb_plan_day_dataEntity>(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void DeleteEntity(string keyValue)
        {
            try
            {
                this.BaseRepository("BaseDb1").Delete<tb_plan_day_dataEntity>(t=>t.id == keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
