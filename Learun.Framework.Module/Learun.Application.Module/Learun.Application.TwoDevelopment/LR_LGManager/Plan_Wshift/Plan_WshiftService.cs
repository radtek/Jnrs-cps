    ...
    public class Plan_WshiftService : RepositoryFactory
    {
        ...
        public IEnumerable<tb_plan_wshift_dataEntity> GetPageList(Pagination pagination, string queryJson)
        {
            try
            {
                ...
                return this.BaseRepository("BaseDb1").FindList<tb_plan_wshift_dataEntity>(strSql.ToString(),dp, pagination);
            }
            catch (Exception ex)
            {
        ...
        public tb_plan_wshift_dataEntity Gettb_plan_wshift_dataEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository("BaseDb1").FindEntity<tb_plan_wshift_dataEntity>(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void DeleteEntity(string keyValue)
        {
            try
            {
                this.BaseRepository("BaseDb1").Delete<tb_plan_wshift_dataEntity>(t=>t.id == keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
