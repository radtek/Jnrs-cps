    ...
    public class OperatorHelper
    {
        ...
        public string AddLoginUser(string account, string mobileCode, string appId, string loginMark, bool cookie = true)
        {
            ...
            try
            {
                ...
                OperateOnlineInfo.cacheKeyToken = cacheKeyToken + account;
                OperateOnlineInfo.tokenMarkList = tokenMarkList;
                OperateOnlineInfo.cacheKeyOperator = cacheKeyOperator + operatorInfo.loginMark;
                OperateOnlineInfo.operatorInfo = operatorInfo;
                OperateOnlineInfo.CacheIdloginInfo = CacheId.loginInfo;
        ...
        public string AddLoginUser(string account, string appId, string loginMark, bool cookie = true)
        {
            ...
            try
            {
                ...
                OperateOnlineInfo.OperateOnlineAccount = account;
                OperateOnlineInfo.OperateOnlineAppId = appId;
                OperateOnlineInfo.OperateOnlineLoginMark = loginMark;
        ...
        public OperatorResult IsOnLine(string token, string loginMark)
        {
            ...
            try
            {
                ...
                if (operatorInfo == null || operatorInfo.token != token)
                {
                    token = AddLoginUser(OperateOnlineInfo.OperateOnlineAccount, OperateOnlineInfo.OperateOnlineAppId, OperateOnlineInfo.OperateOnlineLoginMark);

                    //redisCache.Write<Dictionary<string, string>>(OperateOnlineInfo.cacheKeyToken, OperateOnlineInfo.tokenMarkList, OperateOnlineInfo.CacheIdloginInfo);
                    //redisCache.Write<Operator>(OperateOnlineInfo.cacheKeyOperator, OperateOnlineInfo.operatorInfo, OperateOnlineInfo.CacheIdloginInfo);
                    operatorInfo = OperateOnlineInfo.operatorInfo;
                }
                if (operatorInfo != null && operatorInfo.token == token)
                {
                    TimeSpan span = (TimeSpan)(DateTime.Now - operatorInfo.logTime);
                    if (false && span.TotalHours >= 12)// 登录操作过12小时移除
                    {
